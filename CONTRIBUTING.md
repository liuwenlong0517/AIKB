...
现有分类不是封闭清单。正式知识没有语义匹配的目录时，不得仅为复用现有结构而归入错误分类。应先按内容性质判断它属于通用知识、解决方案、工程陷阱、决策、工作流还是项目事实，再判断是否需要扩展技术领域目录。

同时满足以下条件时，Agent 可以主动创建新分类或主题，无需单独请求确认：

- 当前至少有一条已经满足正式准入标准的真实知识需要归档。
- 新分类的职责边界清晰，名称具有长期稳定性，不依赖某个项目的偶然实现。
- 现有分类均无法在不扭曲语义的情况下容纳该知识。
- 新分类不会立即引发大范围目录迁移或与多个现有分类产生明显重叠。
