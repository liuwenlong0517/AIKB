...
- [ ] Agent 在新会话中只加载一次 `USER_RULES.md`，随后因工程任务触发而读取 `AI_RULES.md` 和 `INDEX.md`。
...
- [ ] 第一个任务只加载 `USER_RULES.md`，没有读取 `AI_RULES.md`、`INDEX.md`、`CATALOG.md` 或具体知识。
...
- [ ] Agent 重新加载个人规则，并因当前任务仍满足接入条件而重新执行基础接入流程和反馈。
- [ ] Agent 仍遵循最小加载原则，不因为上下文丢失扫描整个知识库。
- [ ] 新会话能够加载当前版本的个人规则和索引；如果新任务不满足接入条件，则应按照场景二只加载个人规则。
