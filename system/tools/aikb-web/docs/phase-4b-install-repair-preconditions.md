...
> 文档定位：本文件保留阶段 4B 的历史安全基线、开发波次与验收门槛。阶段 4B 已在 2026-09-01 完成 Windows 用户环境、Codex 和 Claude Code 三个真实目标的独立往返验收；当前能力总览以同目录 `architecture.md`、`api.md`、`security.md` 和 `ui-design.md` 为准。
...
在本文初次冻结时只完成了前置条件和开发波次设计；后续安装与修复已经按本文门槛实现并验收，卸载仍未开放。
...
截至 2026-09-01 开发闭环提交时，阶段 4B 的剩余生产代码已经接通，但尚未执行第 5 波发布验收：
...
上述门槛随后已通过三个真实目标的独立往返验收。完成验收不代表卸载或 macOS 已实现；卸载仍须另建精确删除、恢复和用户授权契约。
