# AIKB WebUI 1.0 架构与读模型

本文档定义 AIKB 1.0 Windows 本地管理终端的当前读模型、受控写入与运行维护边界。知识、运行状态和审计读取保持只读；三项静态本地动作通过任务事实源、SSE 和 Windows Job Object 执行；四项规则可读取且只有 `user` 可受控应用；安装修复只处理三个固定目标；数据维护只处理五类固定终态数据。macOS 仍只保留平台扩展位置，尚未实现或验证。
...
## 数据流与边界
    ...
    React 页面
      ...
      → Web 读模型适配器 / 规则与安装修复事务 / 数据维护规划器 / 受控任务编排器
      → aikb-mcp/aikb 共享核心
...
- 安装修复只消费 `environment`、`agent.codex`、`agent.claude-code` 三个静态目标，通过无副作用预览、逐目标确认、持久化补偿事务和固定 probe 修改 AIKB 受管内容；不接受路径、配置正文、环境值或命令。
- 数据维护只消费五类固定逻辑类别和有界保留期；显式应用与恢复完成后的低频自动维护都在共享锁内重新扫描，只删除仍满足安全终态和保留期条件的对象。
...
数据维护核心只扫描五个固定 workspace 类别：审计数据、终态归档任务、终态 Web 任务、终态规则事务和终态维护事务。服务端保存五分钟短期候选计划；公共 API 只接触逻辑类别、保留天数、计数和摘要。应用取得与规则/安装事务共享的维护写锁后重新扫描，并以候选大小、修改时间和内容摘要拒绝陈旧预览。活动或不确定状态、恢复材料、锁、未知对象及链接/重解析点均不删除。清理成功后只按逻辑 ID 同步任务内存索引和 Working State SQLite 派生行。

应用完成规则与维护事务启动恢复后，后台调度器等待五分钟，再按六小时间隔用服务端固定默认保留期执行同一安全扫描。自动任务不接收浏览器参数、不创建确认令牌，锁忙、扫描超限或对象变化时跳过并留待下一轮；进程退出时由应用生命周期停止调度线程。

完整扫描只允许出现在显式数据维护、启动恢复和显式索引校验中。普通任务列表从随快照写入更新的轻量内存索引分页；Working State Web 列表使用 SQLite 的 `COUNT`、`ORDER BY`、`LIMIT/OFFSET`，归档详情通过索引定位事实文件，不再为每次请求遍历全部 `work.md`。

## 未来扩展点

扩大规则可写范围、增加安装修复目标、索引重建、审计报告生成和知识写入必须新建显式能力契约，不能复用既有任务、规则、安装或数据维护接口暗中扩权。macOS 仍只保留 `platform/base.py` 与扩展目录；React/FastAPI 可移植不等于 macOS Agent hook、权限、路径大小写和 UTF-8 已验证。后续新增平台应实现同一抽象协议并通过真实进程树、编码和权限回归后再声明支持。
