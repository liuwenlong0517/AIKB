...
数据维护核心只扫描固定 workspace 类别，服务端保存五分钟短期候选计划；公共 API 只接触逻辑类别、保留天数、计数和摘要。应用取得与规则/安装事务共享的维护写锁后重新扫描，并以候选大小、修改时间和内容摘要拒绝陈旧预览。活动或不确定状态、恢复材料、锁、未知对象及链接/重解析点均不删除。清理成功后只按逻辑 ID 同步任务内存索引和 Working State SQLite 派生行。
