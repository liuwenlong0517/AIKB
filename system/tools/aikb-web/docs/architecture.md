...
知识、运行状态和审计路由只读。阶段 3 注册受保护的动作预览、任务创建、任务取消和任务事件流；阶段 4A 额外注册规则读取、候选预览、`user` 应用和变更状态查询。所有入口都不能扩展为其他规则/知识写入、Git 写入、索引重建或任意 Shell。未知 /api/* 必须 JSON 404，不能被 SPA 回退吞掉。
