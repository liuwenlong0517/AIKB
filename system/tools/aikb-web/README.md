...
- 数据维护只接受 `audit`、`archived_work`、`web_tasks`、`rule_transactions`、`maintenance_transactions` 五个逻辑类别和 `1..36500` 天保留期；预览及应用均不接收或返回物理路径，应用在全局维护锁内重新扫描，候选变化、链接/重解析点、活动或不确定状态一律拒绝删除；服务启动恢复完成后还会按固定默认保留期低频自动清理，锁冲突或单项校验失败留待下一轮重试；
